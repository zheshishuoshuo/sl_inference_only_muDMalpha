...
# compute_A_one_image_direct.py
# -*- coding: utf-8 -*-
"""
只卡 A 像（B 像放开）的 A(α) 计算器 —— 直接从目标分布采样 logMh（无IS权重）

思想：
- 固定 mu_DM（例如 12.91）
- 生成 (logM_sps, logRe, beta) 人群样本
- 给定 alpha：logM_star = logM_sps + alpha
- 从截断正态 p(logMh | mu_DM, logM_sps) 直接采样每个样本的 logMh
- 前向解两像位置 → 取 μA, μB（只用 μA 进选择函数）
- 对源星等分布积分（ms_grid + pdf_ms）
- 结果 A(α) = 平均的 w_ms（不再乘 p_Mh 权重）

用法：
python -m sl_inference_only_muDMalpha.compute_A_one_image_direct
"""
