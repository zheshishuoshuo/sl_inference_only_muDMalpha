...
def _single_lens_likelihood(
    ...
    logMh = grid.logMh_grid
    logM_star = grid.logM_star
    # detJ = grid.detJ[mask]
    detJ = np.abs(grid.detJ)
    muA = grid.muA
    muB = grid.muB
    beta_w = grid.beta

    if np.isnan(logMh.any()):
        raise ValueError("logMh contains NaN values, check the grid data.")

    if np.isnan(logM_star.any()):
        raise ValueError("logM_star contains NaN values, check the grid data.")

    if np.isnan(detJ.any()):
        raise ValueError("detJ contains NaN values, check the grid data.")

    if np.isnan(muA).any():
        raise ValueError("muA contains NaN values, check the grid data.")

    if np.isnan(muB).any():
        raise ValueError("muB contains NaN values, check the grid data.")

    if np.isnan(beta_w).any():
        raise ValueError("beta_w contains NaN values, check the grid data.")
